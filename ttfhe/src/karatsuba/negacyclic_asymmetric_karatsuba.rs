...
macro_rules! negacyclic_asymmetric_karatsuba {
    ($a:expr, $lhs: ident, $rhs: ident, $res: ident, $name: ident) => {
        let a = unsafe {
            transmute::<&i32, &[i32; $a]>(&$lhs[$a])
        };
        let b = unsafe {
            transmute::<&i32, &[i32; $a]>(&$lhs[0])
        };
        let c = unsafe {
            transmute::<&u64, &[u64; $a]>(&$rhs[$a])
        };
        let d = unsafe {
            transmute::<&u64, &[u64; $a]>(&$rhs[0])
        };

        let mut pad =
            MaybeUninit::<[u64; 2 * $a]>::uninit();
        let a_plus_b = unsafe {
            transmute::<&mut u64, &mut [i32; $a]>(&mut (*pad.as_mut_ptr())[0])
        };
        let c_plus_d = unsafe {
            transmute::<&mut u64, &mut [u64; $a]>(&mut (*pad.as_mut_ptr())[$a])
        };
        ...
        let mut mid =
            MaybeUninit::<[u64; 2 * $a - 1]>::uninit();
        let left = unsafe {
            transmute::<&mut u64, &mut [u64; 2 * $a - 1]>(&mut (*pad.as_mut_ptr())[0])
        };
        let right = unsafe {
            transmute::<&mut u64, &mut [u64; 2 * $a - 1]>(&mut $res[0])
        };
        unsafe {
        ...
        unsafe {
            for i in 0..(2 * $a - 1) {
                (*mid.as_mut_ptr())[i] = (*mid.as_ptr())[i].wrapping_sub(left[i]).wrapping_sub(right[i]);
...
#[inline]
pub fn negacyclic_asymmetric_karatsuba_1024(res: &mut [u64; 1024], lhs: &[i32; 1024], rhs: &[u64; 1024]) {
